...
// Конструктор по умолчанию инициализирует title как nullptr и pages и
// circulation как 0.
Book::Book() : title(nullptr), pages(0), circulation(0) {}

// Конструктор, инициализирующий книгу с заданным названием, количеством страниц
// и тиражом.
...
Book::Book(const Book &other)
    : pages(other.pages), circulation(other.circulation) {
...
Book Book::operator+(const Book &other) const {
  ...
  Book tempBook(newTitle, this->pages + other.pages,
...
// Префиксный оператор инкремента, увеличивающий количество страниц и тираж
// на 1.
Book &Book::operator++() {
  ++pages;
  ++circulation;
...
// Постфиксный оператор инкремента, возвращающий старое состояние объекта перед
// инкрементом.
...
Book::operator int() const { return pages + circulation; }
...
void Book::saveToBinaryFile(const char *filename) const {
  ...
  outFile.write(reinterpret_cast<const char *>(&titleLength),
                sizeof(titleLength));
  outFile.write(title, titleLength);
  outFile.write(reinterpret_cast<const char *>(&pages), sizeof(pages));
  outFile.write(reinterpret_cast<const char *>(&circulation),
                sizeof(circulation));
  outFile.close();
}
...
// Вспомогательная функция для безопасного копирования C-строки, обрабатывающая
// nullptr.
