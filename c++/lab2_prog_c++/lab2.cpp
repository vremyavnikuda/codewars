...
int findMaxElementAddress(int list[], int size)
{
    ...
    {
        // Если текущий элемент больше максимального, обновляем максимальный элемент и его адрес
...
// Функция для добавления элементов массива L2 в массив L1 после первого вхождения элемента E
int addElementsAfterFirstOccurrence(int L1[], int &L1Size, int E, int L2[], int L2Size)
{
    ...
    {
        // Если текущий элемент равен E, обновляем индекс вставки и прерываем цикл
        if (L1[i] == E)
        {
            insertIndex = i + 1;
            break;
    ...
    // Если индекс вставки не равен -1, то есть, если элемент E найден в массиве L1
    if (insertIndex != -1)
    {
        // Цикл по всем элементам массива L2
        for (int i = 0; i < L2Size; i++)
        {
            // Сдвигаем все элементы массива L1, начиная с индекса вставки, на одну позицию вправо
            for (int j = L1Size; j > insertIndex; j--)
            {
                L1[j] = L1[j - 1];
            }
            // Вставляем текущий элемент массива L2 на позицию индекса вставки в массив L1
            L1[insertIndex++] = L2[i];
