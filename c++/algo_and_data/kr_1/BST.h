...
template <class TKey, class TData> class BST {
protected:
...
// Конструктор
template <class TKey, class TData> BST<TKey, TData>::BST() {
  root = nullptr;
  count = 0;
}
...
// Деструктор
template <class TKey, class TData> BST<TKey, TData>::~BST() { Clear(); }

// Очистка дерева
template <class TKey, class TData> void BST<TKey, TData>::Clear() {
  ClearRec(root);
  root = nullptr;
  count = 0;
}

template <class TKey, class TData> void BST<TKey, TData>::ClearRec(Node *t) {
  if (t == nullptr)
    return;
  ClearRec(t->left);
  ClearRec(t->right);
...
// Рекурсивный поиск
template <class TKey, class TData> TData BST<TKey, TData>::Search(TKey k) {
...
template <class TKey, class TData> bool BST<TKey, TData>::Delete(TKey k) {
  bool deleted = false;
...
template <class TKey, class TData> void BST<TKey, TData>::Traverse() {
...
template <class TKey, class TData> int BST<TKey, TData>::CalcHeight(Node *t) {
  if (t == nullptr)
    return 0;
...
template <class TKey, class TData> void BST<TKey, TData>::CalcBalance(Node *t) {
  if (t == nullptr)
    return;
...
template <class TKey, class TData> void BST<TKey, TData>::UpdateBalance() {
  CalcBalance(root);
}

// Вывод структуры дерева
template <class TKey, class TData> void BST<TKey, TData>::Show() {
  if (root == nullptr) {
...
template <class TKey, class TData> BST<TKey, TData>::Iterator::~Iterator() {
  delete[] stack;
}
...
template <class TKey, class TData> void BST<TKey, TData>::Iterator::Begin() {
  top = -1;
  current = nullptr;
...
template <class TKey, class TData> void BST<TKey, TData>::Iterator::Next() {
  if (current == nullptr)
    return;
...
template <class TKey, class TData> bool BST<TKey, TData>::Iterator::IsOff() {
